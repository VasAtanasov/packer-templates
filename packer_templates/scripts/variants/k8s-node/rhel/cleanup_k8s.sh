...
if command -v dnf >/dev/null 2>&1; then
  dnf remove -y gcc cpp make kernel-headers kernel-devel kernel-uek-devel 2>/dev/null || true
else
  yum remove -y gcc cpp make kernel-headers kernel-devel kernel-uek-devel 2>/dev/null || true
fi
